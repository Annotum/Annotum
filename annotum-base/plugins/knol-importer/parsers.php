...
class Kipling_DTD_Parser {
	...
	function parse() {
	$authors = $posts = $categories = $tags = $terms = $post = $author_meta = array();
	...
	foreach (pq('article') as $article) {
		...
		$post['post_content_filtered'] = pq('body', $article)->html();
		$post['post_title'] = pq('title-group', $article)->html();
// TODO
// $post['guid'] = 
		...
		// Generated on insert from post title
		$post['post_name'] = '';
...
// @TODO. Is there ever a chance that this post will not be published?
// Reflect in post_state meta as well.
		...
		foreach (pq('subject', $article) as $category) {
		...
		$default_author_id = 1;
		// Grab the author(s). 
		foreach (pq('contrib') as $contributor) {
		...
		// Funding
		// @TODO support multiple tags
		$funding = pq('funding-statement')->text();
		if (!empty($funding)) {	
	 		$post['post_meta'][] = array(
				'key' => '_anno_funding',
				'value' => $funding,
